...
gulp.task('watch', function () {
    ...
    browserSync.init({
        port: 9999,
        // proxy: "http://localhost:3003", //服务器地址
        server: {
            baseDir: "client/dist" //本地静态服务地址
        }
    });

    gulp.watch('client/src/less/**.less', ['less']);
    gulp.watch('client/src/js/**.js', ['copyjs']);
    gulp.watch('client/src/images/**.*', ['copyImage']);
    gulp.watch('client/src/views/*.html', ['copyHtml']);
})
...
gulp.task('build-copyjs', function () {
    return gulp.src('client/src/js/common/**.js')
        .pipe(gulp.dest('client/dist/js'))
});
...
gulp.task('build-htmlmin', function () {
    return gulp.src('client/src/views/**.html')
        .pipe(htmlmin({ collapseWhitespace: true, removeComments: true })) //设置是否压缩html文件 true
